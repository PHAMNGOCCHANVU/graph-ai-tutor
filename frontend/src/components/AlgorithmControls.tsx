...
export default function AlgorithmControls({ onPlay, onPause, onReset, isPlaying, onSpeedChange }: ControlsProps) {
    ...
    <div className="flex items-center gap-6 p-3 bg-slate-800 rounded-xl border border-slate-700 shadow-2xl w-full">
      ...
      <div className="flex-1 flex items-center gap-4 pl-6 border-l border-slate-700">
        <div className="flex-1 h-1 bg-slate-700 rounded-full overflow-hidden">
          <div className="h-full bg-pink-500 transition-all" style={{ width: `20%` }}></div>
        </div>
        <div className="text-slate-500 text-[10px] font-mono whitespace-nowrap">
    
        </div>
      </div>

     {/* Nút thao tác nhanh 
      <button className="bg-slate-700 hover:bg-slate-600 px-4 py-2 rounded text-[10px] font-bold uppercase tracking-widest transition-all">
        Thay đổi dữ liệu
      </button>*/}
    </div>
  );
